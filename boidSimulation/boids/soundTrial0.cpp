...
struct MyApp : DistributedAppWithState<CommonState> {
  ...
  Parameter DBturnRate{"Dark Blue Turn Rate", "", 0.5, 0.0001, 1.0};
  ...
  void onAnimate(double dt) override {
    ...
    if (isPrimary()) {
      ...
      for (auto& b : boids) {
        ...
        Vec3f desiredDirection = {rand_float(0, CUBE_SIZE), rand_float(0, CUBE_SIZE), rand_float(0, CUBE_SIZE)};
        if (updateRandDir == 10){
          b.bNav.faceToward(b.bNav.pos() + desiredDirection, rand_double(0, 1), turnRate*b.turnRateFactor);
          updateRandDir = 0;
        }
        updateRandDir++;
        b.updatePosition(dt);
  ...
  void setUp() {  
      ...
      for (int i = 0; i < MAX_BOIDS; ++i) {
        ...
        randomize(b.bNav);
        createVoice(b.bNav.x(), b.bNav.y(), b.bNav.z(), b.frequency);
