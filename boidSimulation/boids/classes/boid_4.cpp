...
public:    
    ...
    int get_i_boids_same_type(std::vector<Boid>& boids){
        int count = 0;
        for (int i : i_boids){
            if (boids[i].getType() == this->getType()){
                count++;
            }
        }
        return count;
    ...
    void boidForces(std::vector<Boid>& boids, float alignmentForce = 0.5, float cohesionForce = 0.5, float separationForce = 0.5, float turnRate = 0.5) {
        Vec3f averageHeading(0, 0, 0);
        Vec3f centerOfMass(0, 0, 0);
        Vec3f separation(0, 0, 0);
        Vec3f averageUp(0, 0, 0);
        int count_i_boids_same_type = 0;
        ...
        for (int i : this->i_boids) {
            if (boids[i].getType() == this->getType()){
                ...
                count_i_boids_same_type += 1;
            }
        }

        if (!this->i_boids.empty()) {
            //clean_i_boids(boids);
            averageUp /= count_i_boids_same_type;
            averageHeading /= count_i_boids_same_type;
            centerOfMass /= count_i_boids_same_type;
            separation /= count_i_boids_same_type;
    ...
    void updatePosition(double dt) {
        bNav.moveF(0.67 * speed_percentage);
        bNav.step(dt);        
    }
